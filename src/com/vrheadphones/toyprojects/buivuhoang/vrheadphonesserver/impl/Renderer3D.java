...
/**
 * @author Mark Bernard date: 16-Nov-2003
 *
 *         Port of NeHe's Lesson 5 to LWJGL Title: 3D Shapes Uses version
 *         0.8alpha of LWJGL http://www.lwjgl.org/
 *
 *         Be sure that the LWJGL libraries are in your classpath
 *
 *         Ported directly from the C++ version
 *
 *         2004-05-08: Updated to version 0.9alpha of LWJGL. Changed from all
 *         static to all instance objects. 2004-09-22: Updated to version
 *         0.92alpha of LWJGL.
 */
public class Renderer3D implements Runnable {
	...
	// private static final String VERTEX_SHADER_LOCATION =
	// "res/shaders/vertex_phong_lighting.vs";
	// private static final String FRAGMENT_SHADER_LOCATION =
	...
	public boolean render() throws Exception {
		...
		camera.setRotation((float) Math.toDegrees(thetaY),
				(float) Math.toDegrees(thetaX), (float) Math.toDegrees(thetaZ));
	...
	private void createWindow() throws Exception {
		...
		for (int i = 0; i < d.length; i++) {
			if (d[i].getWidth() == 1280 && d[i].getHeight() == 720
	...
	private static void setUpDisplayLists() {
		try {
			...
			for (int i = 0; i < soundList.size(); i++) {
				...
				float y = sound.y;
				float z = sound.z;
				int display = OBJLoader.createDisplayList(
						OBJLoader.loadModel(sound.getClass().getClassLoader()
								.getResourceAsStream(MODEL_LOCATION)), x, z, y);
	...
	private static void setUpCamera() {
		camera = new EulerCamera.Builder()
				...
				.setPosition(camPosition[0], camPosition[1], camPosition[2])
	...
	private static void setUpLighting() {
		...
		GL11.glLightModel(
				...
				BufferTools.asFlippedFloatBuffer(new float[] { 0.05f, 0.05f,
						0.05f, 1f }));
	...
	public synchronized void setRotationAngle(float thetaX, float thetaY,
			float thetaZ) {
